...
2. **Verify server status**: Confirm the dev server is running and accessible
   - **Auto-restart server**: After code changes, automatically restart the dev server using `run_in_terminal` with `isBackground: true`
   - Check if server is already running on the port (5173-5175)
   - If port is in use, use `get_terminal_output` to verify server health
   - Handle restarts silently in the background without requiring user intervention
...
- [ ] Check if server is responding on 5173-5175 using HTTP request
- [ ] Only restart if server is not responding
- [ ] After restart, verify HTTP response (not just terminal output)
- [ ] Extract actual port from terminal output
- [ ] Test the actual URL before providing it to user
- [ ] Provide clickable server URL in final response

**Automated Server Health Check**: After making code changes:
1. Test `http://localhost:5173` with `Invoke-WebRequest -UseBasicParsing`
2. If responding: Report existing server URL
3. If not responding: Restart following protocol above
4. **Always verify the link works** before giving it to user
...
4. **Run tests** → Verify all tests pass (Vite HMR auto-reloads, no restart needed)
5. **Report results** → Include server URL if relevant

## Vite Hot Module Replacement (HMR)
**DO NOT restart the server after code changes.** Vite automatically detects file changes and hot-reloads the browser. The user will start the server manually if needed.

**CRITICAL: NEVER send any input to the Vite terminal.** The server runs in interactive mode and any input triggers VS Code approval prompts.

## Starting the Dev Server (user action)
When the server needs to be started, instruct the user to run:
```powershell
cd C:\Users\enufa\hddl\hddl-sim; npx vite --port 5173 --logLevel warn
```
The `--logLevel warn` flag suppresses interactive prompt hints.

## Server Health Check (only if needed)
Use `-UseBasicParsing` flag to avoid interactive prompts:
```powershell
Invoke-WebRequest -Uri http://localhost:5173 -UseBasicParsing -TimeoutSec 2 -ErrorAction SilentlyContinue
```

## Required Response Format After Changes
Every response after making code changes MUST include:
1. Brief summary of changes made
2. Test results (X/Y tests passing)
3. Server URL: `http://localhost:5173/`
