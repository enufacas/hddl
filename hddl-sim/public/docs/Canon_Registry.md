# Canon Registry
**Confidence:** Medium

This registry is the authoritative index of all first-class canonical documents in the HDDL corpus.
Any document not listed here is non-canonical.

## Change Log

- **v9.0** — Canon-wide rename to **HDDL (Human-Derived Decision Layer)** and Decision-based system nouns.
  - Standardized: Decision Workspace, Decision Envelope, Decision Authority, Decision Telemetry, Decision Degradation, Decision Promotion
  - Standardized: **DSG** as the stewardship construct for cross-domain arbitration; domain stewards handle domain-scoped calibration
  - Embedded HDDL invariants in System Overview and Executive Reference
  - Removed legacy JSG (Judgment Stewardship Group) terminology

## Foundations
- Glossary.md
- foundations/HDDL_System_Overview.md
- foundations/Executive_Reference.md
- foundations/Decision_Telemetry_Specification.md
- foundations/Decision_Memory_and_AI_Native_Operations.md
- foundations/Human_Derived_Decision_Layer_Foundational_Principles.md

## Stewardship & Roles
- groups/Decision_Stewardship_Group.md
- roles/Engineering_Steward.md
- roles/Domain_Engineer.md
- roles/Business_Domain_Steward.md
- roles/Resiliency_Steward.md
- roles/Data_Steward.md
- roles/Human_Resources_Steward.md
- roles/Sales_Steward.md
- operations/Steward_Led_Decision_Calibration.md
- operations/Continuous_Stewardship.md
- operations/Steward_Playbook.md
- operations/Ceremonies_and_Operating_Cadence.md
- appendices/Envelope_Efficiency_Metrics.md

## Appendices
- appendices/HDDL_vs_AgentOps_vs_AI_Gateway.md
- appendices/HDDL_Simulation_Concept.md
- appendices/Individual_Human_Judgment_Balance_Sheet.md
- operations/Request_Lifecycle_Walkthrough.md

## Legacy / Deprecated (Non-canonical)

- foundations/Human_Derived_Decision_Layer_Manifesto.md
